...
- **Provisioning Scripts**: All provisioning logic is in [`files/`](./files/):
  - [`SetupSsh.ps1`](./files/SetupSsh.ps1): Runs as EC2Launch v2 user data (so its output only reaches `Output.tmp`/`Err.tmp` under the SYSTEM profile's temp folder, never Packer), and records a transcript to `C:\ProgramData\Amazon\EC2Launch\log\SetupSsh-transcript.log` (native command output is piped to `Write-Output` so the transcript captures it). Installs and configures OpenSSH, disables password and keyboard-interactive authentication (`PasswordAuthentication`, `KbdInteractiveAuthentication`, and `ChallengeResponseAuthentication` set to `no`, since older Windows OpenSSH re-enables keyboard-interactive via `ChallengeResponseAuthentication`; prepended to `sshd_config` as ASCII), sets up firewall rules, and schedules a task to fetch the SSH key from EC2 metadata using IMDSv2 (retrieves a session token with 5-minute TTL via `PUT /latest/api/token`, then uses token to fetch SSH key).
  - [`PrepareImage.ps1`](./files/PrepareImage.ps1): Cleans up SSH keys with retry logic (5 attempts with 5-second delays to handle file locks, failing the build if the file still can't be deleted), removes the build-time SSH host keys (`ssh_host_*`) so each launched instance generates unique host keys on first sshd start, removes the `SetupSsh.ps1` transcript (non-fatal if it can't) so it isn't baked into the AMI, ensures scheduled tasks are enabled, and runs Sysprep via EC2Launch. The `PrepareImage.ps1` provisioner accepts `valid_exit_codes = [0, 2300218]`, since `ec2launch sysprep` shuts down the instance and drops the SSH connection before the script can return a normal exit code; Packer surfaces this disconnect as exit code `2300218`, which is treated as success.
...
- **CI/CD**: GitHub Actions workflows in [`.github/workflows/`](./.github/workflows/):
  - [`build-and-test-ami.yml`](./.github/workflows/build-and-test-ami.yml): Comprehensive end-to-end testing on pull requests and pushes to `main`:
    ...
    - Validates and builds AMIs using Packer (version pinned by the workflow's `PACKER_VERSION` env var, which Dependabot doesn't update, so bump it by hand; the runner image's preinstalled Packer is used when it matches, otherwise the release is downloaded from `releases.hashicorp.com` and SHA-256 verified, avoiding `hashicorp/setup-packer`'s dependency on the intermittently failing `api.releases.hashicorp.com`; plugins cached keyed on template hash); CI builds pass `-var "enable_fast_launch=false"` to skip Fast Launch AMI pre-provisioning overhead
    - All Packer and workflow-created AWS resources are tagged `WorkflowRunId=${{ github.run_id }}` to enable safe cancellation
    - Launches test instances (`TEST_INSTANCE_TYPE`, currently `m7i-flex.large`) from the built AMI in a temporary security group that only allows SSH from the runner's public IP, trying each subnet in an AZ that offers that type (looked up with `describe-instance-type-offerings`) in turn if launch fails with `InsufficientInstanceCapacity`; waits for `instance-status-ok` (OS health checks) before attempting SSH, reducing retry flakiness
    - Verifies all AMI snapshots are encrypted
    - Tests SSH connectivity with automatic retry logic (45 attempts, 10-second intervals and a 10-second connect timeout, up to ~15 minutes)
  ...
  - [`PSScriptAnalyzer.yml`](./.github/workflows/PSScriptAnalyzer.yml): Lints every `*.ps1` in the repository (provisioning scripts, Pester tests, and `scripts/`) on pull requests and pushes to `main` that change a `.ps1` file, `PSScriptAnalyzerSettings.psd1`, or the workflow file itself. It runs [`scripts/Invoke-PSScriptAnalyzer.ps1`](./scripts/Invoke-PSScriptAnalyzer.ps1), which pins PSScriptAnalyzer 1.25.0 (preinstalled on `ubuntu-latest`, installed via `Install-PSResource` only if missing), applies the shared [`PSScriptAnalyzerSettings.psd1`](./PSScriptAnalyzerSettings.psd1) (Error and Warning severities), emits each finding as a GitHub annotation so it shows inline on the PR diff, and fails on any finding.
...
- **Script Placement**: All provisioning scripts are in `files/`, referenced directly in the Packer template. Repository tooling scripts (not baked into the image) are in `scripts/`.
- **PSScriptAnalyzer**: Rules and severities live only in `PSScriptAnalyzerSettings.psd1`, and the PSScriptAnalyzer version only in `scripts/Invoke-PSScriptAnalyzer.ps1`; CI, the pre-commit hook, and VS Code all use them. Fix findings rather than excluding rules where practical; use `Write-Output`, not `Write-Host`, in provisioning scripts.
